...
/** Tela de abertura, comum a todos os formulários — o texto vem de cada um. */
export function FormLanding({ form }: { form: BriefingForm }) {
  const { copy } = form;
  ...
  return (
    <main className="mx-auto flex min-h-svh max-w-2xl flex-col justify-center px-5 py-16 sm:px-6 sm:py-20">
      ...
      <p className="type-eyebrow mt-16 text-accent-soft">{copy.eyebrow}</p>

      <h1 className="type-display mt-5 text-paper">{copy.welcomeTitle}</h1>

      <p className="mt-8 max-w-xl text-lg leading-relaxed text-muted sm:text-xl">{copy.welcomeLead}</p>

      <div className="mt-8 max-w-xl space-y-5 text-[0.9375rem] leading-relaxed text-muted">
        {copy.welcomeBody.map((paragraph) => (
          <p key={paragraph}>{paragraph}</p>
        ))}
      </div>

      <p className="mt-8 max-w-xl text-[0.9375rem] leading-relaxed text-paper">{copy.welcomeClosing}</p>
